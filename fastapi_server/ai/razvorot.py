...
async def b_scenario(data: Razvorot2):
    ...
    response = client.chat.completions.create(
        model="gpt-4o",
        messages=[{"role": "user",
                "content":f"Мы - психологический сайт,который помогает людям разобраться в свои взаимоотношениях с другими людьми,сайт помогает поменять свою реакцию на действия этих людей методом психологического Разворота.Представь,что ты психолог и тебе надо проверить корректность формата ответов пользователя на конкретные вопросы.Ты должен проверить,что каждое предложение соответствует правилам русского языка,а каждое слово является осмысленным,а не просто набором букв.Также убедись,что пользователь дает ответ на поставленный вопрос,также проверь,что ни в одном предложении нету имени,отличного от {data0.who}.Дальше ты увидишь данные в формате: <Вопрос?-Ответ пользователя>\nДанные:\n<Какое ваше неэфективное качество?-{data.scenery}>\n<Какова ваша позитивная реакция примерно противоположная этой '{data0.reaction}'?-{data.positive_reaction}>\nВерни «Да» если все ответы пользователя соответствуют описанным мною правилам,иначе верни вопрос,на который пользователь дал некорректный ответ."
        }],
           timeout=100
    )
    title = response.choices[0].message.content
