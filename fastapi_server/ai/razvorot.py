...
async def b_scenario(data0:Razvorot1,data: Razvorot2):
    ...
    response = giga.chat(f"учти ты не можешь изменять это предложение '{data.scenery}' ,верни 'да', если ('{data.scenery}' объясняет мое конкретное эмоциональное отношение к '{data0.who}' без добавления слов или догадок и уточняет эмоцианальную окраску и есть выражения каких-либо конкретных чувств ) И  ('{data.scenery}' является понятным и предложением и соответствует нормам русского языка) И  (каждое слово из '{data.scenery}' имеет смысл ,иначе верни 'нет') И (среди '{data.scenery}' есть прилагательное)  иначе верни 'нет'")
